...
按任务加载上下文：默认只读本文件、改动侧的相邻实现和测试；只有需求或历史决策影响当前行为时，才读取对应 `docs/`、`PRD.md` 或历史计划。
...
- 每个实现任务及其提交只围绕一个可验证行为；非生成代码改动过大时拆分阶段，无法拆分则说明其原子性。
- 代码改动只有在运行最小相关验证后才算完成；最终交接必须列出准确命令、结果和未验证缺口。
- 长周期改造每个线程只推进一个已确认阶段；阶段结束时留下“决策、改动文件、剩余任务、最后验证基线”的简短交接。

当前前端 `package.json` 没有 lint/test 脚本；`npm run verify:frontend` 的构建成功只证明可编译和打包，不代表行为测试已覆盖，交接必须明确这一缺口。
